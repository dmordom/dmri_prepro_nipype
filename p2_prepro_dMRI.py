...
def do_p2_prepro_dMRI(subject_ID, freesurfer_dir, data_dir, data_template, workflow_dir, output_dir):
    ...
    from paux import threshold_bval
    from paux import pick_full_ribbon
    from paux import get_voxels
    from paux import assign_voxel_ids
    from paux import get_mean_b0
    from paux import generate_lateral_line
    ...
    dmripipeline = pe.Workflow(name='p2_prepro_dMRI')
    ...
    bvalbvec_rename = pe.Node(util.Rename(format_string='bval_bvec.txt'), name='01_bvalbvec_rename')
    dmripipeline.connect(fsl2mrtrix, "encoding_file", bvalbvec_rename, "in_file")
    ...
    eddy_corrected_dmri_rename = pe.Node(util.Rename(format_string=subject_ID +'_diff_moco.nii'), name='01_eddy_corrected_dmrirename')
    dmripipeline.connect(eddy_corrected_dmri, 'outputnode.eddy_corrected', eddy_corrected_dmri_rename, "in_file")
    ...
    ribbon_nii.inputs.out_file = subject_ID + '_ribbon_t1.nii'
    ...
    flirt_t1masked_2_FAmasked.inputs.out_file = subject_ID + '_t1_masked_linear2fa.nii'
    ...
    #t1_warped_2_fa.inputs.output_image = subject_ID + '_t1_masked_warped2fa.nii'
    ...
    t1_warped_2_fa_rename = pe.Node(util.Rename(format_string=subject_ID  + '_t1_warped2fa.nii'), name='21_t1_warped_2_fa_rename')
    dmripipeline.connect(t1_warped_2_fa, 'output_image', t1_warped_2_fa_rename, "in_file")

    """
    # warp ribbon 2 fa
    """
    ribbon_warped_2_fa = pe.Node (interface=ants.WarpImageMultiTransform(), name='21_ribbon_warped_2_fa')
    #ribbon_warped_2_fa.inputs.output_image = subject_ID + '_ribbon_dwi.nii'
    ...
    ribbon_warped_2_fa_rename = pe.Node(util.Rename(format_string=subject_ID  + '_ribbon_warped2fa.nii'), name='21_ribbon_warped_2_fa_rename')
    dmripipeline.connect(ribbon_warped_2_fa, 'output_image', ribbon_warped_2_fa_rename, "in_file")
    ...
    """
    find center coordinates of ribbon-masked image
    """
    
    lateral_line = pe.Node(interface=Function(input_names=["image_file"], output_names=["out_file"], function=generate_lateral_line), name='24b_lateral_line')
    lateral_line.run_without_submitting = True
    dmripipeline.connect([(fa_nl_ribbon_masked, lateral_line, [("out_file", "image_file")])])
    ...
    dmripipeline.connect([(lateral_line, fa_wm_connectedcomp_mask, [('out_file', 'operand_file')])])
...
#     direction_prior.inputs.out_file = "directions_100.txt"
    ...
    dmripipeline.connect(lateral_line,'out_file', interface_connectedcomp_mask, 'operand_file')
    ...
    interface_voxels_left = pe.Node(interface=Function(input_names=["interface_file","outfile_prefix"], output_names=["voxel_file","mm_file","mrtrix_file","voxel_list"], function=get_voxels), name='54_interface_voxels_left')
    ...
    dmripipeline.connect(eddy_corrected_dmri_rename, 'out_file', datasink, 'diff_data')
    ...
    dmripipeline.connect(bvalbvec_rename, 'out_file', datasink, 'diff_data.@4')
    ...
    dmripipeline.connect(ribbon_warped_2_fa_rename, 'out_file', datasink, 'anatomy.@8')
    dmripipeline.connect(t1_warped_2_fa_rename, 'out_file', datasink, 'anatomy.@9')
    ...
    dmripipeline.connect(lateral_line, 'out_file', datasink, 'fa_masking.@20')
    ...
    connectprepro = pe.Workflow(name="p2_prepro_dMRI")
